...
from pyflink.datastream.connectors import FlinkKafkaConsumer, FlinkKafkaProducer # type: ignore
import json
from pyflink.datastream import RuntimeContext # type: ignore
from pyflink.common import  Types # type: ignore
import pickle
import redis # type: ignore
from datetime import datetime, timedelta
from river.anomaly import HalfSpaceTrees # type: ignore
from river import preprocessing # type: ignore 
...
KAFKA_PREDICTION_OUTPUT_TOPIC = "Predictions"
KAFKA_METRICS_OUTPUT_TOPIC = "Metrics"
...
KAFKA_PRODUCER_TOPIC = "Predictions"
...
class FraudClassifier(KeyedProcessFunction):

    def open(self, runtime_context):
        self.redis = redis.Redis(**REDIS_CONFIG)
        model_key = "global_models:fraud_hst_model"
        model_bytes = self.redis.get(model_key)
        self.model = pickle.loads(model_bytes) if model_bytes else HalfSpaceTrees(seed=42)

    def process_element(self, value, ctx):
        data = json.loads(value)
        Rules_Check = int(data["Rules_Check"])
        # Build feature dict for river
        features = {
            f"rule{i+1}": float(data.get(f"score_rule{i+1}", 0.0))
            for i in range(8)
        }
        ...
        is_anomaly = score > FRAUD_DETECTION_THRESHOLD
        is_anomaly = Rules_Check == 1 
        data["fraud_prediction"] = int(is_anomaly)
        data["fraud_score"] = round(score, 4)

        # Train model
        self.model.learn_one(features)

        # Persist updated model
        self.redis.set("global_models:fraud_hst_model", pickle.dumps(self.model))

        yield json.dumps(data)

class RealTimeMetricProcessFunction(KeyedProcessFunction):
    ...
    def process_element(self, value, ctx):
        ...
        actual = int(record["isFraud"])
        predicted = int(record["fraud_prediction"])

        # Read current counts from Redis or default to 0
        tp = int(self.redis.get("latest:tp") or 0)
        fp = int(self.redis.get("latest:fp") or 0)
        tn = int(self.redis.get("latest:tn") or 0)
        fn = int(self.redis.get("latest:fn") or 0)

        # Update counters
        if actual == 1 and predicted == 1:
            tp += 1
            self.redis.set("latest:tp", tp)
        elif actual == 0 and predicted == 1:
            fp += 1
            self.redis.set("latest:fp", fp)
        elif actual == 0 and predicted == 0:
            tn += 1
            self.redis.set("latest:tn", tn)
        elif actual == 1 and predicted == 0:
            fn += 1
            self.redis.set("latest:fn", fn)
...
def main():
    ...
    RuleCheckScores = (
        source_stream
        .key_by(lambda x: json.loads(x)["transaction_id"])
    ...
    FraudScores = (
        RuleCheckScores
        .key_by(lambda x: json.loads(x)["transaction_id"])
        .process(UnifiedScoring(), output_type=Types.STRING())
    )

    FraudClassified = (
        FraudScores
        .key_by(lambda x: json.loads(x)["txn_id"])
        .process(FraudClassifier(), output_type=Types.STRING())
    )

    # === Metrics Sink ===
    metrics_stream = FraudClassified \
        .key_by(lambda x: json.loads(x)["txn_id"]) \
        .process(RealTimeMetricProcessFunction(), output_type=Types.STRING())

    metrics_producer = FlinkKafkaProducer(
        topic=KAFKA_METRICS_OUTPUT_TOPIC,
        serialization_schema=SimpleStringSchema(),
        producer_config=kafka_producer_props
    )
    metrics_stream.add_sink(metrics_producer).name("Kafka Metrics Sink")

    # === Predictions Sink ===
    prediction_producer = FlinkKafkaProducer(
        topic=KAFKA_PREDICTION_OUTPUT_TOPIC,
        serialization_schema=SimpleStringSchema(),
        producer_config=kafka_producer_props
    )
    FraudClassified.add_sink(prediction_producer).name("Kafka Prediction Sink")
